...
# =========================
# 3. FETCH DATA (With Anti-Block Bypass)
# =========================
@st.cache_data(ttl=300, show_spinner=False) # Cached for 5 minutes
def fetch_market_data(ticker_symbol):
    ...
    # 2. Fetch Price (Disguised as a real browser to bypass Yahoo Finance blocks)
    try:
...
# =========================
# 4. LOAD, PREDICT, AND DELETE (Automated)
# =========================
data = []
week_ago = datetime.now().date() - timedelta(days=7)

with st.spinner("Loading Distilled AI Model & Analyzing Sentiments..."):
    # Load smaller model locally
    ...
    # 🔴 CRITICAL MEMORY SAVER: Delete model from RAM instantly so it doesn't crash the server
    del sentiment_model
    gc.collect()
...
with st.spinner("Dashboard is live. Next automated refresh in 5 minutes..."):
    time.sleep(300) # Pause execution for 300 seconds (5 minutes)
    st.rerun()      # Tell Streamlit to reload the entire script automatically
